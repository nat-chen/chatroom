...
export function Chat() {
  ...
  useEffect(() => {
    ...
    // 建立与服务器的连接，两种消息类型：1. 加入房间 joinRoom 2. 发送消息 sendMessage
    socket.on("connect", function () {
      ...
      socket.emit("joinRoom", payload); // 发送加入房间的消息
      ...
      socket.on("message", (reply: Reply) => {
        ...
        if (reply.type === "sendMessage") {
          setChatHistory((chatHistory) => {
            return chatHistory
              ? [...chatHistory, reply.message] // 历史消息 + 新消息
              : [reply.message];
          });
          // 收到消息后滚动到底部
          setTimeout(() => {
  ...
  // 消息可能为文本、图片、文件，但最终都是字符串（图片与文件为 url 地址）
  function sendMessage(value: string, type: MessageType = "text") {
    ...
    socketRef.current?.emit("sendMessage", payload); // 发送消息后，on("message") 监听到服务端发送过来的消息
  }
